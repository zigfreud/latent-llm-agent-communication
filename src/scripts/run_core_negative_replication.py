"""Run one frozen H0-012 negative-coverage replication cell."""
...
from src.pipelines.core_negative_replication import (
    run_core_negative_replication_training,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--experiment-config",
        type=Path,
        default=Path("config/LIP-H0-012_core_negative_replication.yaml"),
    )
    parser.add_argument(
        "--parent-config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("experiments/registry/LIP-H0-011_core_negative_coverage.json"),
    )
...
def main() -> None:
    ...
    if args.dry_run_contract:
        print("LIP-H0-012 contract validated")
        return
    if args.finalize_compute_units_after is not None:
        ...
        print(args.summary)
        return
    if args.bundle_dir is None or args.output_dir is None or args.seed is None:
        raise ValueError("--bundle-dir, --output-dir, and --seed are required")
    if int(args.seed) not in (4001, 4003):
        raise ValueError("H0-012 seed must be 4001 or 4003")
    result = run_core_negative_replication_training(
        experiment_path=args.experiment_config,
        parent_path=args.parent_config,
        predecessor_registry_path=args.predecessor_registry,
        bundle_dir=args.bundle_dir,
        output_dir=args.output_dir,
        variant_name="large_negative_batch_unrolled",
        seed=int(args.seed),
        pilot=False,
