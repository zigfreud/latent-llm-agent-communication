"""Generate the frozen text-only capability screen for LIP-PROTO-010."""
...
def run_screening(
    ...
    *,
    ...
    data = config["data"]
    if int(data["candidate_task_count"]) != ORACLE_CAPABILITY_CANDIDATE_COUNT:
        raise ValueError("candidate count does not match the frozen screen")
    ...
    if tuple(seeds) != ORACLE_CAPABILITY_SCREENING_SEEDS:
        raise ValueError("screening seeds do not match the frozen screen")
    expected_keys = {
        (str(task["task_id"]), ORACLE_CAPABILITY_SCREENING_CONDITION, seed)
    ...
    if resume and output_path.exists():
        ...
        if any(
            row.get("protocol_version") != ORACLE_CAPABILITY_PROTOCOL_VERSION
            or row.get("design_sha256") != design_sha256
            or row.get("run_scope") != ORACLE_CAPABILITY_SCREENING_SCOPE
    ...
    print("Loading target model for capability screening...")
    ...
    with output_path.open(output_mode, encoding="utf-8") as output_handle:
        for generation_seed in seeds:
            for task_index, task in enumerate(candidates):
                ...
                key = (task_id, ORACLE_CAPABILITY_SCREENING_CONDITION, generation_seed)
                if key in existing_keys:
                    continue
                formatted, inputs = encoded[task_index]
                effective_seed = stable_seed(generation_seed, task_index, 110)
                set_seed(effective_seed)
                print(
                    f"Screening seed={generation_seed} task={task_id} "
                    f"condition={ORACLE_CAPABILITY_SCREENING_CONDITION}"
                )
                ...
                record = {
                    ...
                    "run_scope": ORACLE_CAPABILITY_SCREENING_SCOPE,
                    "claim_eligible": False,
                    "task_id": task_id,
                    "functional_split": ORACLE_CAPABILITY_SCREENING_SCOPE,
                    "condition": ORACLE_CAPABILITY_SCREENING_CONDITION,
    ...
    metadata = {
        ...
        "conditions": [ORACLE_CAPABILITY_SCREENING_CONDITION],
...
def main() -> None:
    ...
    print("Capability screening generation completed")
