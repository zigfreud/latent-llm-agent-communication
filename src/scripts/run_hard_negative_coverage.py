"""Run the frozen H0-015 hard-negative batch pilot or screen."""
...
from src.pipelines.hard_negative_coverage import (
    build_hard_negative_batch_plan,
    run_hard_negative_coverage_training,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--experiment-config",
        type=Path,
        default=Path("config/LIP-H0-015_hard_negative_batches.yaml"),
    )
    parser.add_argument(
        "--parent-config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("experiments/registry/LIP-H0-014_core_only_margin.json"),
    )
...
def main() -> None:
    ...
    if args.dry_run_contract:
        print("LIP-H0-015 contract validated")
        return
    ...
    if args.plan_only:
        _, metadata = build_hard_negative_batch_plan(
            experiment, args.candidate_bank
        )
        print(json.dumps({k: v for k, v in metadata.items() if k != "batches"}))
        return
    if args.bundle_dir is None or args.output_dir is None:
        raise ValueError("--bundle-dir and --output-dir are required")
    result = run_hard_negative_coverage_training(
        experiment_path=args.experiment_config,
        parent_path=args.parent_config,
        predecessor_registry_path=args.predecessor_registry,
