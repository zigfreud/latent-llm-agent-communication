"""Run the frozen H0-011 core negative-coverage pilot or screen."""
...
from src.pipelines.core_negative_coverage import (
    run_core_negative_coverage_training,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--experiment-config",
        type=Path,
        default=Path("config/LIP-H0-011_core_negative_coverage.yaml"),
    )
    parser.add_argument(
        "--parent-config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "experiments/registry/LIP-H0-010_initial_condition_bridge_matrix_v3.json"
        ),
    )
...
def main() -> None:
    ...
    if args.dry_run_contract:
        print("LIP-H0-011 contract validated")
        return
    ...
    result = run_core_negative_coverage_training(
        experiment_path=args.experiment_config,
        parent_path=args.parent_config,
        predecessor_registry_path=args.predecessor_registry,
