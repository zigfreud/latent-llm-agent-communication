"""Run the frozen H0-014 core-only-margin pilot or screen."""
...
from src.pipelines.core_only_margin import (
    run_core_only_margin_training,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--experiment-config",
        type=Path,
        default=Path("config/LIP-H0-014_core_only_margin.yaml"),
    )
    parser.add_argument(
        "--parent-config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("experiments/registry/LIP-EVAL-032_gradient_geometry.json"),
    )
...
def main() -> None:
    ...
    if args.dry_run_contract:
        print("LIP-H0-014 contract validated")
        return
    ...
    result = run_core_only_margin_training(
        experiment_path=args.experiment_config,
        parent_path=args.parent_config,
        predecessor_registry_path=args.predecessor_registry,
        bundle_dir=args.bundle_dir,
        output_dir=args.output_dir,
        variant_name="core_only_margin_unrolled",
        seed=4007,
