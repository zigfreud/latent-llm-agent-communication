...
def apply_overrides(cfg, experiment_id=None, output_dir=None, max_steps=None, device=None):
    if experiment_id:
        ...
        cfg['experiment_id'] = cfg.get('experiment_id') or cfg.get('experiment_name', 'experiment')
...
def write_run_artifacts(output_dir, cfg, metrics, rows):
    ...
    lines = [
        f"# {cfg['experiment_id']} Remote CPU Smoke",
        "",
...
def train(config_path, experiment_id=None, output_dir=None, max_steps=None, device=None):
    ...
    for epoch in range(start_epoch, cfg['training']['epochs']):
        ...
        for batch_idx, (src, tgt) in enumerate(loader):
            ...
            optimizer.zero_grad()
            output = model(src)
            loss, nce, mse, acc = criterion(output, tgt)
            loss.backward()
            optimizer.step()
